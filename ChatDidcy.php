...
class Chat{
    ...
    private $user  = '367644'; //"333811"
    private $password   = "davidansong12345"; 
    private $database  = "367644";  //"333811"          
	...
	public function getUserChat($from_user_id, $to_user_id) {
		...
		$conversation .= "</ul>";
		return array("conversation"=>$conversation, "count"=>$this->getMessageCount($from_user_id, $to_user_id));
	}
	public function showUserChat($from_user_id, $to_user_id) {		
		...
		$sqlUserUpdate = "
			...
			WHERE userid = '".$from_user_id."'"; //'".$to_user_id."'
		mysqli_query($this->dbConnect, $sqlUserUpdate);		
		$data = array(
	...
	public function fetchIsTypeStatus($userId){
		...
		return array("message"=>$output, "state"=>$status);
	}		
	...
	public function getMessageCount($senderUserid, $recieverUserid) {
		$sqlQuery = "
			SELECT * FROM ".$this->chatTable."  
			WHERE sender_userid = '$senderUserid' AND reciever_userid = '$recieverUserid' AND status = '1'";
		$numRows = $this->getNumRows($sqlQuery);
