...
const ap_html = '<style>.ap_container{transition-duration:.5s;display:inline-block;'+
'text-align:center;overflow:hidden;padding:5px;background-color:rgba(245,236,242);'+
'border-radius:4px;transition:all 1s}.ap_controls{width:100%;'+
'display:flex;position:relative;flex-direction:row;font-size:large;'+
'font-family:Tahoma,Geneva,Verdana,sans-serif;cursor:pointer}.ap_controls span'+
'{margin:5px;display:inline-block}.ap_container .playpausebtn{border-radius:50%;'+
'font-size:larger}.ap_container svg{width:25px;height:25px}.ap_controls .duration'+
'{width:80%;margin-right:10px;bottom:4px}.ap_controls input[type=range]'+
'{-webkit-appearance:none;width:100%;outline:0;height:2px;margin:0 15px;background-color:#0ff}'+
'.ap_controls:hover input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;'+
'height:20px;width:20px;border-radius:50%;cursor:pointer;background-color:#00ced1}'+
'.ap_controls input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;height:0}'+
'.ap_sound{padding:4px}.ap_time{margin:4px}</style><div class="ap_container" id="ap_container"> '+
'<div class="ap_controls"> <span class="playpausebtn" onclick="justplay()">'+
' </span><span class="ap_loop" onclick="ap_loopf()"></span> <span class="duration">'+
' <input type="range" min="0" max="100" value="0" class="duration_slider" '+
'onchange="change_duration()"> </span> <span class="ap_time" style="color:black"></span> '+
'<a id="ap_dnl" style="text-decoration:none" title="Pull it back"><span class="ap_download">'+
'</span></a> <span class="ap_sound" '+
'onclick="ap_mute()"> </span> </div> </div>';
...
//Constants
function AudiPlay(audiPlayID = 0){
ap_audio = o('.audiplay');
createcontainer();

document.getElementById("ap_dnl").setAttribute("href", 
ap_audio.src);

document.getElementById("ap_dnl").setAttribute("download", 
ap_audio.src);

if(audiPlayID == 1){
	document.getElementById("ap_container").style.width = "100%";
	document.getElementById("ap_container").style.left = "0%";
}
play = o('.playpausebtn');
ap_loop = o('.ap_loop');
...
track = o('.audiplay');
showtimer = o('.ap_time');
ap_time = ap_audio.currentTime;
Playing_song = false;
playing_time = '  '+secondsToMinutes(ap_audio.duration);
